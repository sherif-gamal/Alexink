class RawMaterialsController < ApplicationController
  ...
  def permit
    if (@raw_material.in_stock >= params['quantity'].to_f)
      ...
      @raw_material.save
      permission = Permission.create!({transaction_type: 2, transaction_id: @raw_material.id, quantity: params['quantity']})
      production = Production.create!({raw_material_id: @raw_material.id, user_name: current_user.name, quantity:params['quantity'].to_f, permission_id: permission.id})
